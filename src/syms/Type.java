...
public abstract class Type {
    ...
    public static class EnumerationType extends ScalarType {
         ...
         */
        private final List<EnumerationElement> elements;
        /**
        ...
        public void addOperators() {
            ...
            scope.addOperator(Operator.LESS_OP, loc, relOpType);
            scope.addOperator(Operator.GREATER_OP, loc, relOpType);
            scope.addOperator(Operator.LEQUALS_OP, loc, relOpType);
            scope.addOperator(Operator.GEQUALS_OP, loc, relOpType);
            Type.FunctionType enumOpType =
                    new Type.FunctionType(this, this);
            scope.addOperator(Operator.PRED_OP, loc, enumOpType);
            scope.addOperator(Operator.SUCC_OP, loc, enumOpType);
        }

        @Override
        public String toString() {
            StringBuilder s = new StringBuilder("enumeration(" + staticLevel + ") {");
            String sep = "";
            for (EnumerationElement entry : elements) {
                s.append(sep).append(entry.ident);
                sep = ",";
            }
            return s + "}";
        }
